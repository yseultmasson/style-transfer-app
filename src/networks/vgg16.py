...
class Vgg16(nn.Module):
    def __init__(self, mode="generative_st"):
        ...
        self.mode = mode
        features = models.vgg16(
            weights=models.VGG16_Weights.IMAGENET1K_V1
        ).features  # load pretrained VGG16 network and collect its features.
        self.feat = features
        ...
        layer_indices = [
            ...
            23,
            26,
        ]  # the indices of the layers we will use
        self.relu_names = [
            "1_1",
            "1_2",
            "2_1",
            ...
            "4_3",
            "5_1",
        ]  # the names of the layers that we will use.

        for i in range(len(layer_indices) - 1):
            layer_start, layer_end = layer_indices[i], layer_indices[i + 1]
            setattr(
                self,
                f"to_relu_{self.relu_names[i]}",
                nn.Sequential(*features[layer_start:layer_end]),
            )

        if self.mode == "generative_st":
            self.extracted_fmaps = [
                "1_2",
                "2_2",
                "3_3",
                "4_3",
            ]  # the feature maps used for the generative approach
        else:
            self.extracted_fmaps = [
                "1_1",
                "2_1",
    ...
    def forward(self, x):
        ...
        for i in range(len(self.relu_names)):
            ...
            if (self.mode == "generative_st") and (
                len(feature_maps) == len(self.extracted_fmaps)
            ):
                return tuple(
                    feature_maps
